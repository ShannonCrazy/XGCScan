//
//  XGCMainAppFuncCodeFormViewController.m
//  iPadDemo
//
...
- (void)reloadSection:(XGCMainFormSectionDescriptor *)section withRowAnimation:(UITableViewRowAnimation)animation {
    [self reloadSections:[NSArray arrayWithObjects:section, nil] withRowAnimation:animation];
}
...
- (void)reloadSections:(NSArray<__kindof XGCMainFormSectionDescriptor *> *)sections withRowAnimation:(UITableViewRowAnimation)animation {
    ...
    [sections enumerateObjectsUsingBlock:^(__kindof XGCMainFormSectionDescriptor * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSUInteger value = [self.descriptor.sections indexOfObject:obj];
        if (value != NSNotFound) {
    ...
    if (self.view.window) {
        ...
        [self.needReloadIndexSets addIndexes:indexSet];
    }
}

- (void)reloadRowsAtRow:(__kindof XGCMainFormRowDescriptor *)row withRowAnimation:(UITableViewRowAnimation)animation {
    [self reloadRowsAtRows:[NSArray arrayWithObjects:row, nil] withRowAnimation:animation];
}
    ...
    for (XGCMainFormSectionDescriptor *section in self.descriptor.sections) {
        for (__kindof XGCMainFormRowDescriptor *row in section.rows) {
            ...
            if ([mediaDescriptor isKindOfClass:[XGCMainFormRowMediaDescriptor class]] && mediaDescriptor.fileJsons.count == 0) {
                flag = NO;
                [self.view makeToast:mediaDescriptor.placeholder ?: [NSString stringWithFormat:@"请添加%@", mediaDescriptor.cDescription] position:XGCToastViewPositionCenter];
                break;
            }
        }
        if (!flag) {
...
- (void)addDictMapPickerViewAction:(XGCMainFormRowDictMapActionDescriptor *)dictMapDescriptor {
    ...
    XGCAlertView *view = [[XGCAlertView alloc] initWithFrame:self.view.frame];
    view.title = dictMapDescriptor.placeholder;
    view.cCodes = [dictMapDescriptor.cCode componentsSeparatedByString:@","];
    view.allowsMultipleSelection = dictMapDescriptor.allowsMultipleSelection;
    view.dictMaps = dictMapDescriptor.dictMaps ?: [XGCUM.cMenu cDictMap:dictMapDescriptor.cCoder];
    __weak typeof(self) this = self;
    view.didSelectDictMapsAction = ^(XGCAlertView * _Nonnull alertView, NSArray<XGCUserDictMapModel *> * _Nonnull dictMaps) {
